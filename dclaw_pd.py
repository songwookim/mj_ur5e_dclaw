...
import numpy as np
model = mj.MjModel.from_xml_path("./universal_robots_ur5e_with_dclaw/robel_sim/dclaw/dclaw3xh.xml")
...
goal = [-0.045, -0.1, 0.15] #Desire position
...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    while viewer.is_running():
        if (np.linalg.norm(x_error) >= tol):
            ...
            I = np.identity(n)
            # xvel = jacp@data.qvel 
            # F_imp = k_d*xvel + k_p*x_error
            # tau_imp = jacp.T @ F_imp
            # data.qfrc_applied[0:9] = tau_imp*1
            ...
            while step < 2:
                mj.mj_integratePos(model, data.qpos, data.qvel, h)
                mj.mj_forward(model, data)
                mujoco.mj_jac(model, data, Jh, jacr, goal, end_effector_id)
                step += 1
            Jdot = (Jh-jacp)/h
            ...
            F_imp = k_p*(jacp.T@xvel) + k_d*(jacp.T@x_error)
            # tau_imp = jacp.T @ F_imp
            data.qfrc_applied[0:9] = F_imp #tau_imp*1
            ...
            # # j_inv = j_inv*20
            # # x_error = x_error * 20
            # grad = 0.1*j_inv @ x_error # finger1: 5,6,7  finger 2: 9,10,11 finger 3:13,14,15
            # data.qpos += 10 * grad
